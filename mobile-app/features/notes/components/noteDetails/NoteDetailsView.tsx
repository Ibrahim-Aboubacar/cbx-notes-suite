...
export const NoteDetailsView = memo(({ note }: { note: TDetailedNote }) => {
    ...
    return (
        <motion.div className="flex flex-col gap-2">
            ...
            <hr className="my-4 border-teal-600/10" />
            <motion.div className="flex gap-2">
                <motion.div
                    ...
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    className="prose prose-teal min-h-[calc(100%-4.5rem-5rem-2rem)] max-w-none flex-1 p-3 pb-20">
                    ...
                    {showFriends && isOwner && note?.sharedWith?.length > 0 && (
                        <motion.div
                            ...
                            <div className="sticky top-1 w-[14rem]">
                                <div className="flex items-center h-8 p-2 text-neutral-500">
                                    <h3 className="text-lg font-semibold ">Amis(es) ({note?.sharedWith?.length})</h3>
                                </div>
                                <div className="flex flex-col flex-1 gap-2 px-2 mt-3">
                                    {note?.sharedWith?.map((user) => (
                                        <div key={user.id} className="flex items-center gap-2 p-2 border rounded-xl">
                                            <span className="flex items-center justify-center rounded-full bg-primary text-primary-foreground size-9">{user.pseudo.charAt(0).toUpperCase()}</span>
                                            <div className="flex flex-col">
                                                <p className="text-sm font-medium leading-3 text-nowrap text-neutral-600">{user.pseudo}</p>
                                                <p className="text-xs font-medium text-nowrap text-neutral-500">{user.email}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
